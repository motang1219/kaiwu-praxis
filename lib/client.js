...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function PlazaOverlay(props) {
      ...
      // 无工作区时的兜底（A 方案实际落地）：DSH 的会话/输入框围绕工作区设计，
      // 未分组会话虽然能创建但输入框仍是工作区选择器。因此这里自动在工作目录下
      // 建一个「开物临时对话」工作区，再走 DSH 原生 startSession(workspaceId)
      // 打开空白会话；随后 sessions.list 变化会触发 applyStaged 选中对应员工。
      function startWithoutWorkspace(id) {
        if (!api || !api.host || !createWorkspace) {
          startSession();
          return;
        }
        setBusy(true);
        setError(null);
        api.host.describe({}).then(function (resp) {
          ...
          // 优先在 cwd 下建专用目录，失败则直接用 cwd 作为工作区路径
          return api.host.createDirectory({ path: base, name: "开物临时对话" }).then(function (cresp) {
            return cresp && cresp.result && cresp.result.ok ? cresp.result.value.path : base;
          }).catch(function () {
            return base;
          });
        }).then(function (dir) {
