...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function PlazaOverlay(props) {
      ...
      // A 方案：没有工作区时也能发起临时对话。
      // 无空白会话时先查工作区；若一个工作区都没有，直接创建带
      // agentPreset 的未分组会话（sessions.create 的 workspaceId 可选），
      // 再调用 DSH 原生 open 打开它，绕开「必须先创建工作区」的僵局。
      function startWithoutWorkspace(id) {
        if (!api || !api.sessions || !api.workspace || !openSession) {
          startSession();
          return;
        }
        setBusy(true);
        setError(null);
        api.workspace.list({}).then(function (resp) {
          var items = resp && resp.result && resp.result.ok && resp.result.value ? resp.result.value.items || [] : [];
          if (items.length > 0) {
            setBusy(false);
            startSession();
            return;
          }
          return api.sessions.create({ agentPreset: id }).then(function (resp2) {
            setBusy(false);
            if (resp2 && resp2.result && resp2.result.ok) {
              stagedRef.current = undefined;
              openSession(resp2.result.value.sessionId);
              setPlazaOpen(false);
            } else {
              setError(resp2 && resp2.result && resp2.result.error ? resp2.result.error.message : "sessions.create failed");
            }
          }).catch(function (e) {
            setBusy(false);
            setError(messageOf(e));
            ...
            inject: function () {
              return {
                ...
                openSession: function (sessionId) {
                  if (scope.workspaces && typeof scope.workspaces.open === "function") {
                    scope.workspaces.open(sessionId);
                  }
                }
              };
            }
