...
export function apply(ctx, config = {}) {
  ctx.inject(['settings'], (sctx) => {
    ...
    // 每次启动同步一次：skills 是宿主随包维护的（只读），以 preset 目录为准；
    // knowledge / sops 是用户在管理端编辑的数据，原样保留；
    ...
    for (const id of WORKER_IDS) {
      ...
      merged[id] = {
        ...
        sops: cur.sops || [],
        skills: seeded[id].skills,
      }
    }
    mkdir(tempDir, { recursive: true }).catch((e) => {
      logger?.warn(`kaiwu-praxis: mkdir temp workspace failed: ${(e && e.message) || e}`)
    })
    const nextValue = { workers: merged, tempWorkspacePath: tempDir }
    const changed = JSON.stringify(merged) !== JSON.stringify((current && current.workers) || {}) || (current && current.tempWorkspacePath) !== tempDir
    if (changed) {
